...
class MockAuthProvider(AuthProvider):
    def register(self, db: Session, email: str, password: str, display_name: str = "") -> tuple[User, str]:
        ...
        db.commit()
        db.refresh(user)
        from memory.engine import ensure_default_consents
...
class SupabaseAuthProvider(AuthProvider):
    """Supabase integration — validates JWT from Supabase Auth and syncs local user record."""

    def _sync_user(self, db: Session, email: str, supabase_id: str, display_name: str = "") -> User:
        user = db.query(User).filter(User.email == email).first()
        if not user:
            user = User(id=supabase_id, email=email, password_hash="", display_name=display_name or email.split("@")[0])
            db.add(user)
            db.flush()
            profile = Profile(id=str(uuid.uuid4()), user_id=user.id)
            db.add(profile)
    ...
    def register(self, db: Session, email: str, password: str, display_name: str = "") -> tuple[User, str]:
        if not settings.supabase_url:
            raise NotImplementedError("Set SUPABASE_URL and SUPABASE_SERVICE_KEY for Supabase auth")
        raise NotImplementedError("Use Supabase client SDK on frontend; backend validates Supabase JWT via /auth/me")
